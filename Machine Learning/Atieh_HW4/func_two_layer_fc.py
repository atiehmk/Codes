...
# 2 hidden layers
def inference(images, image_pixels, hidden_units_first, hidden_units_second, classes, reg_constant=0):
  '''Build the model up to where it may be used for inference.

  Args:
      images: Images placeholder (input data).
      image_pixels: Number of pixels per image.
      hidden_units_first: Size of the first (hidden) layer.
      hidden_units_second: Size of the second (hidden) layer.
  ...
  with tf.variable_scope('Layer2'):
    ...
    weights = tf.get_variable(
      name='weights',
      shape=[hidden_units_first, hidden_units_second],
      initializer=tf.truncated_normal_initializer(
        stddev=1.0 / np.sqrt(float(hidden_units_first))),
      regularizer=tf.contrib.layers.l2_regularizer(reg_constant)
    )

    biases = tf.Variable(tf.zeros([hidden_units_second]), name='biases')
  ...
  with tf.variable_scope('Layer3'):
    # Define variables
    weights = tf.get_variable('weights', [hidden_units_second, classes],
      initializer=tf.truncated_normal_initializer(
        stddev=1.0 / np.sqrt(float(hidden_units_second))),
      regularizer=tf.contrib.layers.l2_regularizer(reg_constant))
...
#
#     # Define the layer's output
#     hidden = tf.nn.tanh(tf.matmul(images, weights) + biases)
#
#   # Layer 2
#   with tf.variable_scope('Layer2'):
#     # Define variables
